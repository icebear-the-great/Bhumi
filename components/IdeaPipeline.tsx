    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex gap-4 overflow-x-auto pb-4 kanban-scroll">
        {columns.map(status => (
          <div
            key={status}
            className={`min-w-[300px] flex-1 flex flex-col rounded-xl border transition-colors max-h-full ${status === IdeaStatus.CANCELLED ? 'bg-stone-100 border-stone-200' : 'bg-sand-200/50 border-sand-200'
              }`}
          >
            <div className={`p-3 border-b rounded-t-xl backdrop-blur-sm sticky top-0 z-10 ${status === IdeaStatus.CANCELLED ? 'bg-stone-100 border-stone-200' : 'bg-sand-200 border-sand-200'
              }`}>
              <div className="flex justify-between items-center">
                <h3 className="font-semibold text-sand-700 flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${status === IdeaStatus.NEW ? 'bg-bhumi-400' :
            ...
            <div className="p-3 space-y-3 overflow-y-auto flex-1 kanban-scroll">
              {getIdeasByStatus(status).map(idea => (
                <div
                  key={idea.id}
                  className={`bg-white p-4 rounded-lg shadow-sm border hover:shadow-md transition-all group relative ${idea.status === IdeaStatus.CANCELLED ? 'border-stone-200 opacity-80 hover:opacity-100' : 'border-sand-200'
                    }`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded border ${idea.status === IdeaStatus.CANCELLED ? 'bg-stone-100 text-stone-500 border-stone-200' : getPriorityColor(idea.priority)
                        }`}>
                        {idea.priority}
                      </span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border flex items-center gap-1 ${idea.type === 'Task' ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-amber-50 text-amber-700 border-amber-200'
                        }`}>
                  ...
                  <h4 className={`font-bold mb-1 leading-tight cursor-pointer ${idea.status === IdeaStatus.CANCELLED ? 'text-stone-600 line-through decoration-stone-300' : 'text-bhumi-900 hover:text-bhumi-600'
                    ...
                    {idea.tags.map(tag => {
                      ...
                      return (
                        <span
                          ...
                          className={`text-xs px-2 py-1 rounded cursor-pointer transition-colors border border-transparent ${isSelected
