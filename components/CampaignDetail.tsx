                        ...
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <div className="relative group">
                                <select
                                    ...
                                    className={`text-xs px-3 py-1.5 rounded-full border font-bold uppercase tracking-wider outline-none cursor-pointer appearance-none pr-8 hover:opacity-80 transition-opacity ${campaign.status === 'Active' ? 'bg-bhumi-100 text-bhumi-700 border-bhumi-200' :
            ...
            <div className="flex border-b border-sand-200">
                {['overview', 'content', 'pipeline', 'assets'].map((tab) => (
                    <button
                        ...
                        className={`px-6 py-3 font-medium text-sm transition-all relative ${activeSection === tab
                            ? 'text-bhumi-700'
                ...
                {activeSection === 'pipeline' && (
                    <div className="space-y-6">
                        <div className="flex justify-between items-center bg-sand-50 p-4 rounded-xl border border-sand-200">
                            <div>
                                <h3 className="font-bold text-bhumi-900">Linked Ideas & Tasks</h3>
                                <p className="text-sand-500 text-sm">Review ideas connected to this initiative.</p>
                            </div>
                            <button
                            ...
                            >
                                {ICONS.Link} Link Existing Idea
                            </button>
                        </div>

                        {/* Linker Modal */}
                        {showLinker && (
                            <div className="fixed inset-0 bg-bhumi-900/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                                <div className="bg-white rounded-xl shadow-xl w-full max-w-md border border-sand-200 p-6">
                                    <h3 className="font-bold text-bhumi-900 mb-4">Link an Idea</h3>
                                    <input
                                        autoFocus
                                        value={linkSearch}
                                        onChange={e => setLinkSearch(e.target.value)}
                                        placeholder="Search unlinked ideas..."
                                        className="w-full border border-sand-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-bhumi-500 outline-none mb-4"
                                    />
                                    <div className="max-h-[300px] overflow-y-auto space-y-2">
                                        {unlinkedIdeas.length > 0 ? (
                                            unlinkedIdeas.map(idea => (
                                                <button
                                                    key={idea.id}
                                                    onClick={() => handleLinkIdea(idea)}
                                                    className="w-full text-left p-3 hover:bg-sand-50 rounded-lg border border-transparent hover:border-sand-200 transition-all group"
                                                >
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border flex items-center gap-1 ${idea.type === 'Task' ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-amber-50 text-amber-700 border-amber-200'
                                                            }`}>
                                                            {idea.type === 'Task' ? ICONS.Pipeline : ICONS.Ideas}
                                                            {idea.type || 'Idea'}
                                                        </span>
                                                        <span className="font-bold text-bhumi-900 text-sm">{idea.title}</span>
                                                    </div>
                                                    <div className="text-xs text-sand-500 pl-1">{idea.category}</div>
                                                </button>
                                            ))
                                        ) : (
                                            <div className="text-center text-sand-400 text-sm py-4">
                                                No matching unlinked ideas found.
                                            </div>
                                        )}
                                    </div>
                                    <button onClick={() => setShowLinker(false)} className="mt-4 w-full py-2 text-sand-500 hover:text-sand-800 text-sm">Cancel</button>
                                </div>
                            </div>
                        )}
                                    ...
                                    <div key={idea.id} className="bg-white p-4 rounded-xl border border-sand-200 shadow-sm flex items-center justify-between group">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 text-xs text-sand-500">
                                                ...
                                                <span>•</span>
                                                <span>Owner: {idea.author}</span>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${idea.priority === 'High' ? 'bg-red-50 text-red-600' :
                                                idea.priority === 'Medium' ? 'bg-yellow-50 text-yellow-600' : 'bg-green-50 text-green-600'
                                                }`}>
                                                {idea.priority} Priority
                                            </div>
                                            <button
                                                onClick={(e) => handleUnlinkIdea(e, idea)}
                                                className="text-sand-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"
                                                title="Unlink idea"
                                            >
                                                {ICONS.Unlink}
                                            </button>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {campaign.contentDrafts.map(draft => (
                                    <div key={draft.id} className="bg-white rounded-2xl border border-sand-200 overflow-hidden shadow-sm flex flex-col">
                                        {/* Status Header */}
                                        <div className={`px-4 py-2 border-b border-sand-100 flex justify-between items-center ${draft.status === 'Approved' ? 'bg-green-50' :
                                            draft.status === 'In Review' ? 'bg-yellow-50' :
                                                draft.status === 'Changes Requested' ? 'bg-red-50' : 'bg-sand-50'
                                            }`}>
                                            <span className={`text-xs font-bold uppercase tracking-wider ${draft.status === 'Approved' ? 'text-green-700' :
