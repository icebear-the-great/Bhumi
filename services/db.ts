...
const setLocal = (key: string, data: any) => {
    try {
        ...
        // We might want to throw here to let the UI know, but local storage rarely fails in this context
    }
};
...
export const db = {
    ...
    updateUserStatus: async (id: string, status: 'Active' | 'Inactive'): Promise<User[]> => {
        ...
        if (!firestore) {
            ...
            if (!current.find(u => u.id === id)) throw new Error("User not found in local storage");
